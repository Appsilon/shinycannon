...
class ShinySession(val sessionId: Int,
    ...
    val allowedTokens: HashSet<String> = hashSetOf("WORKERID", "WORKER", "TOKEN", "ROBUST_ID", "SOCKJSID", "SESSION", "UPLOAD_URL", "UPLOAD_JOB_ID")
    ...
    private fun maybeLogin() {
        credentials?.let { (username, password) ->
            if (isProtected(httpUrl)) {
                postLogin(httpUrl, username.plus(workerId), password, cookies = cookieStore)
            } else {
